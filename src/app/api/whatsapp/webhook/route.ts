...
function mapEvolutionWebhookToMeta(body: any): any {
  ...
  // If it's a message status update
  if (body.event === 'messages.update' || body.event === 'MESSAGES_UPDATE') {
    ...
    const statusVal = statusMap[data.status] || 'sent'
    return {
      entry: [{
        id: body.instance,
        changes: [{
          ...
          value: {
            ...
            statuses: [{
              ...
              recipient_id: data.key.remoteJid.split('@')[0]
            }]
          }
        }]
      }]
  ...
  if (body.event === 'messages.upsert' || body.event === 'MESSAGES_UPSERT') {
    ...
    const pushName = data.pushName || from
    ...
    if (msg.conversation) {
      ...
      type = 'audio'
      mediaData = {
        id: msg.audioMessage.url || '',
        mime_type: msg.audioMessage.mimetype || 'audio/mp4'
      }
    } else if (msg.documentMessage) {
      type = 'document'
      mediaData = {
        id: msg.documentMessage.url || '',
      ...
      interactiveData = {
        ...
        button_reply: {
          ...
          title: msg.templateButtonReplyMessage.selectedDisplayText || ''
        }
      }
