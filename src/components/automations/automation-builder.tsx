...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  add_tag: { label: "Adicionar Marcador", icon: Tag, border: "border-l-primary" },
  remove_tag: { label: "Remover Marcador", icon: TagIcon, border: "border-l-primary" },
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova Mensagem Recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "A primeira vez que este contato envia uma mensagem para você (funciona também para contatos adicionados manualmente)",
  },
  { value: "keyword_match", label: "Correspondência de Palavra-chave", hint: "A mensagem contém palavra(s)-chave específicas" },
  { value: "new_contact_created", label: "Novo Contato Criado", hint: "Quando um contato é auto-criado a partir de uma mensagem recebida" },
  { value: "conversation_assigned", label: "Conversa Atribuída", hint: "Quando atribuída a um agente" },
  { value: "tag_added", label: "Marcador Adicionado", hint: "Quando um marcador é adicionado a um contato" },
  { value: "time_based", label: "Baseado em Tempo", hint: "Em um cronograma recorrente" },
]
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        {value && !selected && (
          <option value={value}>{value} (marcador desconhecido)</option>
        )}
      </select>
    </div>
  )
      ...
      >
        ...
        {current && !hasMatch && (
          <option value={current}>
            {templateName} ({language || "desconhecido"}) — fora da lista de aprovados
          </option>
        )}
      </select>
    </FieldBlock>
        ...
        <>
          <FieldBlock label="Assunto">
            <select
              ...
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="tag_presence">Presença de marcador</option>
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    case "send_message":
      return (step.step_config.text as string) || "nenhum texto ainda"
    case "send_template":
      return (step.step_config.template_name as string) || "escolha um modelo"
    case "wait":
      return `${step.step_config.amount ?? "?"} ${step.step_config.unit === "minutes" ? "minuto(s)" : step.step_config.unit === "hours" ? "hora(s)" : step.step_config.unit === "days" ? "dia(s)" : ""}`
    case "condition":
