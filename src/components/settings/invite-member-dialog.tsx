...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Unico Ex account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Unico Ex account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Unico Ex account';
    const message = `Join ${accountName} on Unico Ex using this link (valid for ${result?.expiresInDays} days): ${url}`;
